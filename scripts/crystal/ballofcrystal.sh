...
../cryptominisat5 --bva 0 --scc 0 --sqlitedbover 1 --cldatadumpratio "$RATIO" --cllockdatagen 0.5 --clid --sql 2 --sqlitedb "$FNAMEOUT.db-raw" --drat "$FNAMEOUT.drat" --zero-exit-status "../$FNAME" | tee cms-pred-run.out
...
/usr/bin/time -v ../test_data_quality.py "$FNAMEOUT.db"
...
(
cd "$FNAME-dir"
../cryptominisat5 "../$FNAME" --bva 0 --printsol 0 | tee cms-final-run.out
)
exit
