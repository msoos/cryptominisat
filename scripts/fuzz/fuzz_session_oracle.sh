#!/bin/bash
#
# Create a tmux session with multiple fuzzing windows running fuzz_oracle.py
#
# Usage:
#   ./fuzz_session_oracle.sh [--num N]
#
# Options:
#   --num N    Number of tmux windows to create (default: 24)
#
# Examples:
#   ./fuzz_session_oracle.sh              # 24 windows
#   ./fuzz_session_oracle.sh --num 8     # 8 windows
#
...
tmux new-session -d -s "$SESSION" -c "$DIR" -n "oracle-1"
tmux send-keys -t "$SESSION:1" "$CMD" Enter

# Create remaining windows
for i in $(seq 2 $NUM_WINDOWS); do
  tmux new-window -t "$SESSION" -c "$DIR" -n "oracle-$i"
