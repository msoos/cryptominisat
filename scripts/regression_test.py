...
class Tester:
    ...
    def __init__(self):
        ...
        self.fuzzers = [
            ["../build/tests/sha1-sat/sha1-gen --attack preimage --rounds 18 --cnf",
                "--hash-bits", "--seed"], ["../build/tests/sha1-sat/sha1-gen --xor --attack preimage --rounds 18 --cnf", "--hash-bits", "--seed"]  # , ["build/cnf-fuzz-nossum"]
            #, ["build/largefuzzer"]
            , ["../build/tests/cnf-utils/cnf-fuzz-biere"], ["../build/tests/cnf-utils/sgen4 -unsat -n 50", "-s"], ["../build/tests/cnf-utils//sgen4 -sat -n 50", "-s"], ["../utils/cnf-utils/cnf-fuzz-brummayer.py", "-s"], ["../utils/cnf-utils/cnf-fuzz-xor.py", "--seed"], ["../utils/cnf-utils/multipart.py", "special"]
        ]
    ...
    def check_debug_lib(self, fname):
        ...
        for debugLibPart in range(1, largestPart + 1):
            ...
            unsat, solution, conflict = self.parse_solution_from_output(
                output_lines)
    ...
    def check(self, fname, fnameSolution=None, fnameDrup=None, newVar=False,
        ...
        unsat, solution, _ = self.parse_solution_from_output(
            consoleOutput.split("\n"))
