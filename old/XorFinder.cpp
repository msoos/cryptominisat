...
XorFinder::XorFinder(Solver& _solver, vector<Clause*>& _cls) :
    cls(_cls)
    , solver(_solver)
{
}
...
const bool XorFinder::fullFindXors(const uint32_t minSize, const uint32_t maxSize)
{
    ...
    for (vector<Clause*>::iterator it = cls.begin(), end = cls.end(); it != end; it ++) {
    ...
    uint32_t i = 0;
    for (vector<Clause*>::iterator it = cls.begin(), end = cls.end(); it != end; it++, i++) {
...
const bool XorFinder::findXors(uint32_t& sumLengths)
{
    ...
    while (getNextXor(begin,  end, impair)) {
        ...
        if (x != NULL) solver.xorclauses.push_back(x);
        if (!solver.ok) return false;
...
{
    uint32_t added = 0;
    vector<XorClause*>::iterator i = solver.xorclauses.begin();
    vector<XorClause*>::iterator j = i;
    for (vector<XorClause*>::iterator end = solver.xorclauses.end(); i != end; i++) {
        if ((*i)->size() > 3) {
            *j++ = *i;
            continue;
        }
...
void XorFinder::addXorAsNormal3(XorClause& c)
{
    ...
    if (tmp) solver.clauses.push_back(tmp);
