...
Gaussian::Gaussian(CommandControl& _ccsolver, const GaussConf& _config, const uint32_t _matrix_no, const vector<XorClause*>& _xorclauses) :
        ccsolver(_ccsolver)
        , config(_config)
...
Gaussian::~Gaussian()
{
    for (uint32_t i = 0; i < clauses_toclear.size(); i++)
        ccsolver.clAllocator.clauseFree(clauses_toclear[i].first);
}

inline void Gaussian::set_matrixset_to_cur()
{
    uint32_t level = ccsolver.decisionLevel() / config.only_nth_gauss_save;
...
const bool Gaussian::full_init()
{
    ...
    reset_stats();
    uint32_t last_trail_size = ccsolver.trail.size();
    ...
    while (do_again_gauss) {
        ...
        case conflict:
            #ifdef VERBOSE_DEBUG
            std::cout << "(" << matrix_no << ")conflict at level 0" << std::endl;
            #endif
            ccsolver.ok = false;
            return false;
        case unit_propagation:
        case propagation:
            unit_truths += last_trail_size - ccsolver.trail.size();
            do_again_gauss = true;
            ccsolver.ok = (ccsolver.propagate().isNULL());
            if (!ccsolver.ok) return false;
            break;
        case nothing:
            break;
...
void Gaussian::init()
{
    ...
    matrix_sets.clear();
    matrix_sets.push_back(cur_matrixset);
    gauss_last_level = ccsolver.trail.size();
    messed_matrix_vars_since_reversal = false;
    badlevel = UINT_MAX;

    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")Gaussian init finished." << std::endl;
    #endif
}

uint32_t Gaussian::select_columnorder(vector<uint16_t>& var_to_col, matrixset& origMat)
{
    var_to_col.resize(ccsolver.nVars(), unassigned_col);
    ...
    for (uint32_t i = 0; i != xorclauses.size(); i++) {
        ...
        for (uint32_t i2 = 0; i2 < c.size(); i2++) {
            assert(ccsolver.assigns[c[i2].var()].isUndef());
    ...
    vector<Var> vars(ccsolver.nVars());
    if (!config.orderCols) {
        for (uint32_t i = 0; i < ccsolver.nVars(); i++) {
            vars.push_back(i);
        }
        std::random_shuffle(vars.begin(), vars.end());
    }

    Heap<Solver::VarOrderLt> order_heap(ccsolver.order_heap);
...
{
    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")Filling matrix" << std::endl;
    #endif
    ...
    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")matrix size:" << origMat.num_rows << "," << origMat.num_cols << std::endl;
    #endif
...
{
    #ifdef VERBOSE_DEBUG_MORE
    std::cout << "(" << matrix_no << ")Updating matrix var " << var+1 << " (col " << col << ", m.last_one_in_col[col]: " << m.last_one_in_col[col] << ")" << std::endl;
    std::cout << "m.num_rows:" << m.num_rows << std::endl;
    #endif
    ...
    if (ccsolver.assigns[var].getBool()) {
...
{
    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")Updating matrix." << std::endl;
    #ifdef VERBOSE_DEBUG_MORE
    print_matrix(m);
    #endif
    ...
    assert(ccsolver.decisionLevel() == 0 || check_last_one_in_cols(m));
    #endif
    ...
    for (const Var *it = &m.col_to_var[0], *end = it + m.num_cols; it != end; col++, it++) {
        if (*it != unassigned_var && ccsolver.assigns[*it].isDef()) {
    ...
    #ifdef VERBOSE_DEBUG
    std::cout << "Matrix update finished, updated " << num_updated << " cols" << std::endl;
    #ifdef VERBOSE_DEBUG_MORE
    print_matrix(m);
    #endif
    #endif
    ...
    std::cout << " num_rows diff:" << origMat.num_rows - m.num_rows << std::endl;
    std::cout << "num_cols:" << col_to_var_original.size();
    std::cout << " num_cols diff:" << col_to_var_original.size() - m.col_to_var.size() << std::endl;
    std::cout << "removeable cols:" << m.removeable_cols << std::endl;*/
}
...
{
    if (ccsolver.decisionLevel() >= badlevel)
        return nothing;

    if (messed_matrix_vars_since_reversal) {
        #ifdef VERBOSE_DEBUG
        std::cout << "(" << matrix_no << ")matrix needs copy before update" << std::endl;
        #endif

        const uint32_t level = ccsolver.decisionLevel() / config.only_nth_gauss_save;
        assert(level < matrix_sets.size());
    ...
    if (!cur_matrixset.num_cols || !cur_matrixset.num_rows) {
        badlevel = ccsolver.decisionLevel();
        return ret;
    }

    if (ret == nothing &&
        ccsolver.decisionLevel() % config.only_nth_gauss_save == 0)
        set_matrixset_to_cur();

    #ifdef VERBOSE_DEBUG
    if (ret == nothing)
        std::cout << "(" << matrix_no << ")Useless. ";
    else
        std::cout << "(" << matrix_no << ")Useful. ";
    std::cout << "(" << matrix_no << ")Useful prop in " << ((double)useful_prop/(double)called)*100.0 << "%" << std::endl;
    std::cout << "(" << matrix_no << ")Useful confl in " << ((double)useful_confl/(double)called)*100.0 << "%" << std::endl;
    #endif
...
uint32_t Gaussian::eliminate(matrixset& m, uint32_t& conflict_row)
{
    ...
    std::cout << "m.least_column_changed:" << m.least_column_changed << std::endl;
    #ifdef VERBOSE_DEBUG_MORE
    ...
    if (m.least_column_changed == INT_MAX) {
        #ifdef VERBOSE_DEBUG
        std::cout << "Nothing to eliminate" << std::endl;
        #endif
    ...
    #ifdef VERBOSE_DEBUG
    std::cout << "At while() start: i,j = " << i << ", " << j << std::endl;
    std::cout << "num_rows:" << m.num_rows << " num_cols:" << m.num_cols << std::endl;
    #endif

    if (j > m.num_cols) {
        #ifdef VERBOSE_DEBUG
        std::cout << "Going straight to finish" << std::endl;
        #endif
        goto finish;
    }
    ...
    #ifdef VERBOSE_DEBUG
    std::cout << "Finished elimination" << std::endl;
    std::cout << "Returning with i,j:" << i << ", " << j << "(" << m.num_rows << ", " << m.num_cols << ")" << std::endl;
    #ifdef VERBOSE_DEBUG_MORE
    print_matrix(m);
    print_last_one_in_cols(m);
    #endif
    std::cout << "(" << matrix_no << ")Exchanged:" << no_exchanged << " row additions:" << number_of_row_additions << std::endl;
    #endif
    ...
    for (; col < m.num_cols && row < m.num_rows && row < i ; col++) {
        ...
        if (m.col_to_var[col] == unassigned_var || !m.matrix.getMatrixAt(row)[col]) {
            #ifdef VERBOSE_DEBUG_MORE
            std::cout << "row:" << row << " col:" << col << " m.last_one_in_col[col]-1: " << m.last_one_in_col[col]-1 << std::endl;
            #endif
...
Gaussian::gaussian_ret Gaussian::handle_matrix_confl(PropBy& confl, const matrixset& m, const uint32_t size, const uint32_t maxlevel, const uint32_t best_row)
{
    ...
    const bool wasUndef = m.matrix.getVarsetAt(best_row).fill(tmp_clause, ccsolver.assigns, col_to_var_original);
    release_assert(!wasUndef);

    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")matrix confl clause:"
    << tmp_clause << " , "
    ...
    if (maxlevel != ccsolver.decisionLevel()) {
        ccsolver.cancelUntil(maxlevel);
    }
    const uint32_t curr_dec_level = ccsolver.decisionLevel();
    ...
    if (tmp_clause.size() == 2) {
        ...
        ccsolver.watches[(~lit1).toInt()].push(Watched(lit2, true));
        ccsolver.watches[(~lit2).toInt()].push(Watched(lit1, true));
        ccsolver.numBins++;
        ccsolver.learnts_literals += 2;
        ccsolver.dataSync->signalNewBinClause(lit1, lit2);
        ...
        confl = PropBy(lit1);
        ccsolver.failBinLit = lit2;
    } else {
        Clause* conflPtr = (Clause*)ccsolver.clAllocator.XorClause_new(tmp_clause, xorEqualFalse);
        confl = PropBy(ccsolver.clAllocator.getOffset(conflPtr), 0);
        Clause& cla = *conflPtr;

        uint32_t maxsublevel_at = UINT_MAX;
        for (uint32_t i = 0, size = cla.size(); i != size; i++) if (ccsolver.varData[cla[i].var()].level == curr_dec_level) {
        ...
        #ifdef VERBOSE_DEBUG
        std::cout << "(" << matrix_no << ") || Sublevel of confl: " << maxsublevel << " (due to var:" << cla[maxsublevel_at].var()-1 << ")" << std::endl;
        #endif
...
Gaussian::gaussian_ret Gaussian::handle_matrix_prop_and_confl(matrixset& m, uint32_t last_row, PropBy& confl)
{
    ...
    #ifdef VERBOSE_DEBUG
    std::cout << "Resizing matrix to num_rows = " << last_row << std::endl;
    #endif
    m.num_rows = last_row;
    ...
    for (vector<uint32_t>::iterator prop_row = propagatable_rows.begin(), end = prop_row + propagatable_rows.size(); prop_row != end; prop_row++ ) {
        //this is a "000..1..0000000X" row. I.e. it indicates a propagation
        ret = handle_matrix_prop(m, *prop_row);
        num_props++;
        if (ret == unit_propagation) {
            #ifdef VERBOSE_DEBUG
            std::cout << "(" << matrix_no << ")Unit prop! Breaking from prop examination" << std::endl;
            #endif
            return  unit_propagation;
        }
    }
    #ifdef VERBOSE_DEBUG
    if (num_props > 0) std::cout << "(" << matrix_no << ")Number of props during gauss:" << num_props << std::endl;
    #endif
...
{
    for (int i = ccsolver.trail.size()-1; i >= 0; i --)
        if (ccsolver.trail[i].var() == v) return i;

    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")Oooops! Var " << v+1 << " does not have a sublevel!! (so it must be undefined)" << std::endl;
    #endif
...
{
    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")Canceling until sublevel " << until_sublevel << std::endl;
    #endif
    ...
    for (vector<Gaussian*>::iterator gauss = ccsolver.gauss_matrixes.begin(), end= ccsolver.gauss_matrixes.end(); gauss != end; gauss++)
        ...
        for (int sublevel = ccsolver.trail.size()-1; sublevel >= (int)until_sublevel; sublevel--) {
        Var var  = ccsolver.trail[sublevel].var();
        #ifdef VERBOSE_DEBUG
        std::cout << "(" << matrix_no << ")Canceling var " << var+1 << std::endl;
        #endif

        ccsolver.assigns[var] = l_Undef;
        ccsolver.insertVarOrder(var);
    }
    ccsolver.trail.resize(until_sublevel);

    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")Canceling sublevel finished." << std::endl;
    #endif
}
...
void Gaussian::analyse_confl(const matrixset& m, const uint32_t row, uint32_t& maxlevel, uint32_t& size, uint32_t& best_row) const
{
    ...
    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")matrix conflict found!" << std::endl;
    std::cout << "(" << matrix_no << ")conflict clause's vars: ";
    #ifdef VERBOSE_DEBUG_MORE
    print_matrix_row_with_assigns(m.matrix.getVarsetAt(row));
    std::cout << std::endl;

    std::cout << "(" << matrix_no << ")corresponding matrix's row (should be empty): ";
    ...
    while (true) {
        ...
        assert(real_var < ccsolver.nVars());

        if (ccsolver.varData[real_var].level > this_maxlevel)
            this_maxlevel = ccsolver.varData[real_var].level;
        var++;
        this_size++;
    }
    ...
    if (!(
        ...
        #ifdef VERBOSE_DEBUG
        std::cout << "(" << matrix_no << ")Other found conflict just as good or better.";
        std::cout << "(" << matrix_no << ") || Old maxlevel:" << maxlevel << " new maxlevel:" << this_maxlevel;
        std::cout << "(" << matrix_no << ") || Old size:" << size << " new size:" << this_size << std::endl;
    ...
    #ifdef VERBOSE_DEBUG
    if (maxlevel != std::numeric_limits<uint32_t>::max())
        std::cout << "(" << matrix_no << ")Better conflict found.";
    else
        std::cout << "(" << matrix_no << ")Found a possible conflict.";
...
{
    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")matrix prop" << std::endl;
    #ifdef VERBOSE_DEBUG_MORE
    std::cout << "(" << matrix_no << ")matrix row:" << m.matrix.getMatrixAt(row) << std::endl;
    #endif
    #endif

    bool xorEqualFalse = !m.matrix.getVarsetAt(row).is_true();
    m.matrix.getVarsetAt(row).fill(tmp_clause, ccsolver.assigns, col_to_var_original);
    #ifdef VERBOSE_DEBUG
    std::cout << "(" << matrix_no << ")matrix prop clause: " << tmp_clause << std::endl;
    std::cout << std::endl;
    #endif
    ...
    switch(tmp_clause.size()) {
        case 0:
            ...
            break;
        case 1:
            ccsolver.cancelUntil(0);
            ccsolver.enqueue(tmp_clause[0]);
            return unit_propagation;
        case 2: {
            ccsolver.cancelUntil(0);
            tmp_clause[0] = tmp_clause[0].unsign();
            tmp_clause[1] = tmp_clause[1].unsign();
            XorClause* cl = ccsolver.addXorClauseInt(tmp_clause, xorEqualFalse);
            release_assert(cl == NULL);
            release_assert(ccsolver.ok);
            return unit_propagation;
        }
        default:
            if (ccsolver.decisionLevel() == 0) {
                ccsolver.enqueue(tmp_clause[0]);
                return unit_propagation;
            }
            Clause& cla = *(Clause*)ccsolver.clAllocator.XorClause_new(tmp_clause, xorEqualFalse);
            assert(m.matrix.getMatrixAt(row).is_true() == !cla[0].sign());
            assert(ccsolver.assigns[cla[0].var()].isUndef());

            clauses_toclear.push_back(std::make_pair(&cla, ccsolver.trail.size()-1));
            ccsolver.enqueue(cla[0], PropBy(ccsolver.clAllocator.getOffset(&cla), 0));
            return propagation;
    }
...
llbool Gaussian::find_truths(SearchFuncParams& params, vector<Lit>& learnt_clause)
{
    PropBy confl;

    disable_if_necessary();
    if (should_check_gauss(ccsolver.decisionLevel(), ccsolver.starts)) {
        called++;
        gaussian_ret g = gaussian(confl);
        ...
        case conflict: {
            useful_confl++;
            llbool ret = ccsolver.handle_conflict(params, learnt_clause, confl);
            if (confl.isClause())
                ccsolver.clAllocator.clauseFree(ccsolver.clAllocator.getPointer(confl.getClause()));
        ...
        case unit_conflict: {
            ...
            #endif
            if (ccsolver.value(lit) != l_Undef) {
                assert(ccsolver.value(lit) == l_False);
                #ifdef VERBOSE_DEBUG
...
void Gaussian::print_matrix_row(const T& row) const
{
    ...
    while (true) {
        ...
        else std::cout << col_to_var_original[var]+1 << ", ";
        var++;
    }
    std::cout << "final:" << row.is_true() << std::endl;;
}
...
void Gaussian::print_matrix_row_with_assigns(const T& row) const
{
    ...
    while (true) {
        ...
        if (col == ULONG_MAX) break;
            ...
            std::cout << var+1 << "(" << lbool_to_string(ccsolver.assigns[var]) << ")";
            std::cout << ", ";
        }
        col++;
    }
    if (!row.is_true()) std::cout << "xorEqualFalse";
}
...
void Gaussian::print_stats() const
{
    if (called > 0) {
        ...
        std::cout << " prop: " << std::setprecision(2) << std::setw(5) << ((double)useful_prop/(double)called)*100.0 << "% ";
        std::cout << " confl: " << std::setprecision(2) << std::setw(5) << ((double)useful_confl/(double)called)*100.0 << "% ";
...
{
    std::cout << "matrix size: " << cur_matrixset.num_rows << "  x " << cur_matrixset.num_cols << std::endl;
}
...
bool Gaussian::check_no_conflict(matrixset& m) const
{
    ...
    for(PackedMatrix::iterator r = m.matrix.beginMatrix(), end = m.matrix.endMatrix(); r != end; ++r, ++row) {
        if ((*r).is_true() && (*r).isZero()) {
            std::cout << "Conflict at row " << row << std::endl;
            return false;
        }
    }
    return true;
...
void Gaussian::print_matrix(matrixset& m) const
{
    ...
    for (PackedMatrix::iterator it = m.matrix.beginMatrix(); it != m.matrix.endMatrix(); ++it, row++) {
        std::cout << *it << " -- row:" << row;
        if (row >= m.num_rows)
            std::cout << " (considered past the end)";
        std::cout << std::endl;
    }
}
...
void Gaussian::print_last_one_in_cols(matrixset& m) const
{
    for (uint32_t i = 0; i < m.num_cols; i++) {
        std::cout << "last_one_in_col[" << i << "]-1 = " << m.last_one_in_col[i]-1 << std::endl;
    }
}
...
const bool Gaussian::nothing_to_propagate(matrixset& m) const
{
    for(PackedMatrix::iterator r = m.matrix.beginMatrix(), end = m.matrix.endMatrix(); r != end; ++r) {
        if ((*r).popcnt_is_one()
            && ccsolver.assigns[m.col_to_var[(*r).scan(0)]].isUndef()) {
            #ifdef VERBOSE_DEBUG
...
void Gaussian::check_matrix_against_varset(PackedMatrix& matrix, const matrixset& m) const
{
    for (uint32_t i = 0; i < matrix.getSize(); i++) {
        ...
        while (true) {
            ...
            assert(var < ccsolver.nVars());

            if (ccsolver.assigns[var] == l_True) {
                assert(!mat_row[col]);
                assert(m.col_to_var[col] == unassigned_var);
                assert(m.var_is_set[var]);
                final = !final;
            } else if (ccsolver.assigns[var] == l_False) {
                assert(!mat_row[col]);
                assert(m.col_to_var[col] == unassigned_var);
                assert(m.var_is_set[var]);
            } else if (ccsolver.assigns[var] == l_Undef) {
                assert(m.col_to_var[col] != unassigned_var);
                assert(!m.var_is_set[var]);
                assert(mat_row[col]);
            } else assert(false);
        ...
        if ((final^!mat_row.is_true()) != !var_row.is_true()) {
            std::cout << "problem with row:"; print_matrix_row_with_assigns(var_row); std::cout << std::endl;
            assert(false);
        }
    }
}
...
const void Gaussian::check_first_one_in_row(matrixset& m, const uint32_t j)
{
    if (j) {
        ...
        if (j-1 > m.first_one_in_row[m.num_rows-1]) {
            until2 = m.num_rows;
            #ifdef VERBOSE_DEBUG
            std::cout << "j-1 > m.first_one_in_row[m.num_rows-1]" << "j:" << j << " m.first_one_in_row[m.num_rows-1]:" << m.first_one_in_row[m.num_rows-1] << std::endl;
            #endif
        }
        for (uint32_t i2 = 0; i2 != until2; i2++) {
            #ifdef VERBOSE_DEBUG
            std::cout << std::endl << "row " << i2 << " (num rows:" << m.num_rows << ")" << std::endl;
            std::cout << m.matrix.getMatrixAt(i2) << std::endl;
            std::cout << " m.first_one_in_row[m.num_rows-1]:" << m.first_one_in_row[m.num_rows-1] << std::endl;
            std::cout << "first_one_in_row:" << m.first_one_in_row[i2] << std::endl;
            std::cout << "num_cols:" << m.num_cols << std::endl;
            std::cout << "popcnt:" << m.matrix.getMatrixAt(i2).popcnt() << std::endl;
            std::cout << "popcnt_is_one():" << m.matrix.getMatrixAt(i2).popcnt_is_one() << std::endl;
            std::cout << "popcnt_is_one("<< m.first_one_in_row[i2] <<"): " << m.matrix.getMatrixAt(i2).popcnt_is_one(m.first_one_in_row[i2]) << std::endl;
            #endif
...
/*void Gaussian::update_matrix_by_row(matrixset& m) const
{
    ...
    for (uint32_t col = 0; col < m.num_cols; col ++) {
        ...
        if (var != UINT_MAX && !ccsolver.assigns[var].isUndef()) {
            toclear.setBit(col);
            if (ccsolver.assigns[var].getBool()) tocount.setBit(col);
...
#ifdef VERBOSE_DEBUG
    std::cout << "Updated " << num_updated << " matrix cols. Could remove " << m.removeable_cols << " cols " <<endl;
#endif
}*/
...
/*void Gaussian::update_matrix_by_col(matrixset& m, const uint32_t last_level) const
{
    ...
    for (int level = ccsolver.trail.size()-1; level >= last_level; level--){
        Var var = ccsolver.trail[level].var();
...
#ifdef VERBOSE_DEBUG
    std::cout << "Updated " << num_updated << " matrix cols. Could remove " << m.removeable_cols << " cols (out of " << m.num_cols << " )" <<endl;
#endif
}*/
