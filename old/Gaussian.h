...
{
public:
    Gaussian(CommandControl& ccsolver, const GaussConf& config, const uint32_t matrix_no, const vector<XorClause*>& xorclauses);
    ~Gaussian();

    const bool full_init();
    llbool find_truths(SearchFuncParams& params, vector<Lit>& learnt_clause);
...
inline void Gaussian::canceling(const uint32_t sublevel)
{
    ...
    if (messed_matrix_vars_since_reversal)
        return;
    int c = std::min((int)gauss_last_level, (int)(ccsolver.trail.size())-1);
    for (; c >= (int)sublevel; c--) {
        Var var  = ccsolver.trail[c].var();
