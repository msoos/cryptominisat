...
// CMS addition: expose the internal tick counter (cleared by kitten_clear)
// so embedders can budget effort without reaching into kitten internals.
uint64_t
kitten_current_ticks (kitten * kitten)
{
  return KITTEN_TICKS;
}
...
#if defined(STAND_ALONE_KITTEN) && !defined(KITTEN_NO_MAIN)
