...
/* CMS: 'yals_compute_weighted_break' is commented out upstream, so this
   checker cannot be compiled as it stands. */
static void yals_check_xweighted_breaks(Yals * yals) { (void) yals; }
...
static void yals_connect (Yals * yals) {
  ...
  if (yals->opts.crit.val) {
    ...
    /* CMS: 'crit' doubles as the flag for computing break values on the fly,
       so it must not end up NULL on a formula made up of XOR clauses only. */
    NEWN (yals->crit, nclauses + 1);
...
void yals_del (Yals * yals) {
  ...
  if (yals->crit) DELN (yals->crit, yals->nclauses + 1);
