...
void Main::parseCommandLine()
{
    ...
    ("solprint,s", po::value<int>(&printResult)->default_value(printResult)
    ...
    ("nocleanabove", po::value<uint32_t>(&conf.clauseCleanNeverCleanAtOrAboveThisPropConfl)->default_value(conf.clauseCleanNeverCleanAtOrAboveThisPropConfl)
        , "Don't clean a clause if its sum prop+confl is at or higher than this value")
    ...
    ("clearpropconfl", po::value<int>(&conf.doClearPropConfEveryClauseCleaning)->default_value(conf.doClearPropConfEveryClauseCleaning)
        , "Clear prop&confl data of each clause after clause cleaning")
    ...
    ("rewardotfsubsume", po::value<int>(&conf.rewardShortenedClauseWithPropConfl)
        ->default_value(conf.rewardShortenedClauseWithPropConfl)
    ...
    ("cachecut", po::value<size_t>(&conf.cacheUpdateCutoff)->default_value(conf.cacheUpdateCutoff)
        , "Don't update the cache for a literal when it propagates more than this many literals")
