...
void Conglomerate::blockVars(vector<pair<XorClause*, uint32_t> >& clauseSet, const Var var)
{
    for (uint i = 0; i != clauseSet.size(); i++) {
        for (Lit  *i2 = clauseSet[i].first->getData(), *end = i2 + clauseSet[i].first->size() ; i2 != end; i2++)
            if (i2->var() != var)
                blocked[i2->var()] = true;
    }
}
